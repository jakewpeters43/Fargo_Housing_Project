...
```{r}
ggplot() + geom_point(data=FM_Housing_Clean[which(FM_Housing_Clean$`Sold Price` <= 700000) & FM_Housing_Clean$`Sold Price` > 10000,], mapping=aes(x=`Geo Lon`, y=`Geo Lat`, color=log(`Sold Price`)))+ scale_color_gradient(low="yellow",high="red") + geom_point(data=FM_Housing_Clean[which(FM_Housing_Clean$`Sold Price` > 700000),], mapping=aes(x=`Geo Lon`, y=`Geo Lat`), size=0.5) + ylim(46.75, 46.95) + xlim(-96.95, -96.7)
```

```{r}
ggplot(FM_Housing_Clean[which(FM_Housing_Clean$`Sold Price` <= 500000) & FM_Housing_Clean$`Sold Price` >= 10000,]) + geom_histogram(mapping=aes(x=log(`Sold Price`), base=1.1))
```
