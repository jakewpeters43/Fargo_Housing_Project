...
FM_Housing_Raw <- rbind(FM_Housing_Raw1, FM_Housing_Raw2)
...
```{r}
FM_Housing_Clean_v1 <- FM_Housing_Raw_v1[!is.na(FM_Housing_Raw_v1$`Geo Lon`) & !is.na(FM_Housing_Raw_v1$`Geo Lat`),]
FM_Housing_Clean_v1 <- FM_Housing_Clean_v1 %>% cbind("Census Tract"=FM_Tract_Raw$tractvec)
FM_Housing_Clean <- FM_Housing_Raw %>% left_join(FM_Housing_Clean_v1 %>% select(c("Geo Lat", "Geo Lon", "Census Tract")), by=c("Geo Lat", "Geo Lon"))

FM_Housing_Clean %>% filter(is.na(`Census Tract`) & !is.na(`Geo Lat`) & !is.na(`Geo Lon`)) %>% select(c("Geo Lat","Geo Lon"))
```

```{r}
FM_Housing_Clean <- FM_Housing_Raw[!is.na(FM_Housing_Raw$`Geo Lon`) & !is.na(FM_Housing_Raw$`Geo Lat`),]
#FM_Housing_Clean <- FM_Housing_Clean %>% left_join(FM_Tract_Raw, by=c("Geo Lat", "Geo Lon"))
...
FM_Housing_Clean <- FM_Housing_Clean %>% mutate("New Construction"=if_else(`New Construction Type`=="No", 0, 1))
...
FM_Housing_Clean <- FM_Housing_Clean %>% mutate("Census Tract Group" = if_else(`Sales in Tract`>100, `Census Tract`, "Other"))
...
```{r}
ggplot(data=FM_Agg_Monthly %>% filter(Period>=as.Date("2005-01-01"), Period < as.Date("2020-01-01")), mapping=aes(x=Period)) + geom_line(mapping=aes(y=`Seasonally-corrected MRI`, color="Months Remaining Inventory"), size=.75) + geom_line(mapping=aes(y=`Unemployment-adjusted Affordability Index`/100, color="Affordability"), size=.75) + theme_bw() +  scale_color_manual("", 
...
FM_Housing_Clean %>% group_by(`List Period`) %>% summarize("New Properties Built"=sum(`New Construction`)) %>% ggplot(aes(`List Period`, `New Properties Built`)) + geom_line() + geom_smooth() + xlim(as.Date("2000-01-01"),as.Date("2020-05-01"))
```
