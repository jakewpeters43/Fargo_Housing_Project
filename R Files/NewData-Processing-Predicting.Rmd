...
New_Data_Clean$`Hedonic Prediction` <- round(exp(predict(hedonicModel, newdata=New_Data_Clean, allow.new.levels=TRUE)),-2)
```
...
adjustmentModel <- lmer(`Log Price`~
                        `Log List Price`+
                        `Log SqFt`+
                        `Year Built`+
                        `Building Age`+
...
New_Data_Clean$`Listing Adjustment Prediction` <- round(exp(predict(adjustmentModel, newdata=New_Data_Clean, allow.new.levels=TRUE)),-2)
```

```{r}
New_Data_Clean %>% write_csv("../Data/FM_Market_Clean.csv") #specify new file name for data sources other than FM_on_Market
```
