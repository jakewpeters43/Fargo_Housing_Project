...
FM_Housing_Clean <- FM_Housing_Clean %>% mutate("Log Price"=log(`Adjusted Price`)) %>% filter(`Adjusted Price` > 0)
...
```{r}
plotFEsim(FEsim(trainingModel, n.sims = 100), level = 0.9, stat = 'median', intercept = FALSE)
plotREsim(REsim(trainingModel, n.sims = 100), stat = 'median', sd = TRUE)
...
ggplot(FM_Housing_Clean) + geom_col(mapping=aes(x=`Census Tract Group`, y=`Real Sold Price`))
...
FM_Housing_Clean <- FM_Housing_Clean %>% mutate("Adjusted CPI"=100*`CPI`/261.560`)
#ggplot(FM_Housing_Clean) + geom_line(mapping=aes(x=`Start Date`, y=`Price Index`), color="blue") + geom_line(mapping=aes(x=`Start Date`, y=`Adjusted CPI`))
#ggplot(FM_Housing_Clean) + geom_point(mapping=aes(x=`Start Date`, y=`Adjusted Price`), alpha=0.1) + geom_hline(yintercept=269563.3, color="blue")
