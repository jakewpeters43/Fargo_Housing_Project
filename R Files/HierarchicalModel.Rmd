...
```{r}
coefficients <- seq(-0.004, -0.001, 0.0001)
rvals <- vector(length=n)
rvals_recent <- vector(length=n)

for (k in 1:length(coefficients)) {
  FM_Housing_Clean <- FM_Housing_Clean %>% mutate("Model Weight"=exp(coefficients[k]*`Days Since Listing`))
...
rvals[k] <- mean(resamples)
rvals_recent[k] <- mean(resamples_recent)
}

ggplot(data.frame(coefficients, rvals, rvals_recent)) + geom_line(mapping=aes(x=coefficients, y=rvals), color="red") + geom_line(mapping=aes(x=coefficients, y=rvals_recent), color="blue")
```