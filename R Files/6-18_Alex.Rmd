...
hlm1 <- lmer(log(`Sold Price`)~
                    ...
                    (1|`Postal Code`),
                    data=FM_Housing_Clean, REML=FALSE)
summary(hlm1)
```
...
for(i in 1:n) {
  ...
  housingLinear <- lmer(log(`Sold Price`)~
                        `Total SqFt.`+
                        `Year Built`+
                        `Start Date`+
                        `Total Bedrooms`+
                    ...
                    (1|`Postal Code`),
                    data=FM_Housing_Clean, REML=FALSE)
  testedModel <- housingLinear %>% predict(FM_Housing_Test, allow.new.levels=TRUE)
