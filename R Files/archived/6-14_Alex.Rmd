...
FM_Housing_Clean <- FM_Housing_Clean %>% left_join(Features_Clean %>% filter(V2=="# Stalls Attached") %>% select("Index", V3)) %>% replace_na(list(V3=0)) %>% rename("Attached Stalls"=V3) %>% distinct(Index, .keep_all=TRUE) 
...
```{r}
linear2fold <- train(log(`Sold Price`)~
                       `Affordability Index`+
                       `Total SqFt.`+
                       `Months Remaining Inventory`+
                       `Start Date`+`Year Built`+
                       `Total Bedrooms`+
                       `Garage Stalls`+
                       `Median Market Price in Period`+
                       `Median Sold Price in Period`+
                       `City Water`+
                       `Has Air Conditioning`+
                       `Dishwasher Included`+
                       `Style`,
              data=filter(FM_Housing_Clean, `Pended Date`>=as.Date("2002-01-01")&`Total SqFt.`>0),
              method="lm",
