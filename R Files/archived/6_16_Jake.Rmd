...
FM_Housing_Clean <- FM_Housing_Clean %>% mutate("Is New Construction"=if_else(`New Construction`!="No","Yes", "No"))
```
...
for(i in 1:n) {
  ...
  housingLinear <- lm(log(`Sold Price`)~
                      ...
                      + `Is New Construction`+
                      `Foundation`,
                    data=filter(FM_Housing_Clean, `Total SqFt.`>0))
...
ggplot(data=FM_Housing_Clean, mapping = aes(y=`Geo Lat`, x=`Geo Lon`, color = `Map Area`)) + geom_point()
```
```{r}
write.csv(FM_Housing_Clean,"C:/Users/13204/Documents/FM_Housing_Clean.csv", row.names = FALSE)
...
get_tract <-  function(x,y) {
 return(tract)
  ...
  #draft <- read_html(paste0("https://www.basketball-reference.com/draft/NBA_",year,".html"))
  

  
  lat_lon <- read_html(paste0("https://geocoding.geo.census.gov/geocoder/geographies/coordinates?x=-96.8256&y=46.8243&benchmark=4&vintage=4"))
