...
```{r}
FM_Housing_filter <-  FM_Housing_Clean %>% filter(`Year Built`>1869) 
ggplot(data = FM_Housing_filter) + geom_histogram(mapping = aes(x=`Year Built`)) 
ggplot(data=FM_Housing_Clean) + geom_point(mapping = aes(x=`Geo Lon`,y=`Geo Lat`, color = `Book Section`)) + xlim(-96.925,-96.72) + ylim(46.76,46.935)

ggplot(data = FM_Housing_Clean) + geom_point(mapping=aes(x= `Months Remaining Inventory`, y=`Sold Price`))
```
