...
```{r}
housingLinear <- lm(log(`Sold Price`, 1.1)~`Book Section`+`Start Date`+`Map Area`+`Total SqFt.`+`Year Built`+`Style`+`Total Bedrooms`+`Total Bathrooms`+`Garage Type`+`Gen Tax`+`Map Zone`+`Flood Plain`+`School District`+`Lot Size SqFt`+`Garage Stalls`+`High School`+`Days on Market`+`Air Conditioning`+`Foundation`+`Exterior`+`Refrigerator Included`+`Dishwasher Included`+`Washer Included`+`Dryer Included`+`New Construction`+`Roof`+`City Water`+`City Sewer`+`Water Heater`, data=FM_Housing_Clean,na.action = na.exclude)
summary(housingLinear)
```

```{r}
FM_Housing_Clean <- FM_Housing_Clean %>% mutate("Has AC"= if_else(`Air Conditioning` == "None", "No", "Yes"))
...
yale <- function(n){
  result = ifelse(n<5, "Harvard", "Yale")
  return(result)
}
yale(4)
...
FM_Housing_Clean %>% distinct(`Year Built`)
```
