...
```{r}
# kmeans Clustering with just sold price
FM_Housing_kmeans <- FM_Housing_Clean %>% mutate(logprice = log(`Sold Price`, base = 1.1)) #%>% mutate()

# K-Means Cluster Analysis
fit <- Kmeans(FM_Housing_kmeans$`logprice`, 21, nstart = 2, method = "manhattan") # 21 cluster solution
# get cluster means
aggregate(FM_Housing_kmeans$`logprice`,by=list(fit$cluster),FUN=mean)
# append cluster assignment
FM_Housing_kmeans$`logprice` <- data.frame(FM_Housing_kmeans$`logprice`, fit$cluster)

fit
FM_Housing_kmeans <- FM_Housing_kmeans %>% cbind(as.factor(fit$cluster)) %>% rename("cluster" = `as.factor(fit$cluster)`)
...
```{r}
ggplot() + geom_point(data=FM_Housing_Clean, mapping=aes(x=`Geo Lon`, y=`Geo Lat`), color=FM_Housing_Clean$`cluster`) + scale_color_gradient(low="yellow",high="red") + ylim(46.75, 46.95) + xlim(-96.95, -96.7)
